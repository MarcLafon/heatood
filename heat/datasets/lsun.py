...
class LSUNClass(VisionDataset):
    def __init__(
            self, root: str, label_lmdb: str,
            transform: Optional[Callable] = None,
            target_transform: Optional[Callable] = None
    ) -> NoneType:
        import lmdb

        root = lib.expand_path(root)

        super(LSUNClass, self).__init__(os.path.join(root, label_lmdb),
                                        transform=transform,
                                        target_transform=target_transform)

        self.env = lmdb.open(os.path.join(root, label_lmdb), max_readers=1, readonly=True, lock=False,
                             readahead=False, meminit=False)
        with self.env.begin(write=False) as txn:
            self.length = txn.stat()['entries']
        cache_file = os.path.join(root, '_cache_' + ''.join(c for c in label_lmdb if c in string.ascii_letters + "_"))
